...
const playgroundsList = [
  ...
  { id: 6, tag: "06", title: "Interactive Magnetic Orbit Nodes", desc: "Interactive floating magnetic energy nodes with spring tension hover effects." }
];
...
export default function Lab() {
  ...
  // --- EXPERIMENT 6: MAGNETIC ORBIT NODES ---
          ...
          <div className="lg:col-span-7 bg-[#121212]/80 dark:bg-[#121212]/80 light:bg-white backdrop-blur-2xl p-6 md:p-8 rounded-[2.5rem] border border-white/10 dark:border-white/10 light:border-slate-300 shadow-2xl flex flex-col justify-between relative overflow-hidden min-h-[420px] transition-colors duration-300">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {activeId === 2 && (
                  <div className="w-full h-[360px] bg-[#1a1a1a] dark:bg-[#1a1a1a] light:bg-slate-100 rounded-3xl border border-white/10 dark:border-white/10 light:border-slate-300 p-6 flex flex-col justify-center items-center gap-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 w-full">
                      <button onClick={() => triggerPad('glitch')} className={`synth-pad-btn p-4 rounded-xl border font-mono text-xs uppercase font-bold transition-all ${activePad === 'glitch' ? 'active-pad bg-[#f5e156] text-black border-[#f5e156]' : 'bg-[#181818] text-white border-white/10'}`}>01 // Glitch Saw</button>
                      <button onClick={() => triggerPad('sub')} className={`synth-pad-btn p-4 rounded-xl border font-mono text-xs uppercase font-bold transition-all ${activePad === 'sub' ? 'active-pad bg-[#f5e156] text-black border-[#f5e156]' : 'bg-[#181818] text-white border-white/10'}`}>02 // Sub Sine</button>
                      <button onClick={() => triggerPad('laser')} className={`synth-pad-btn p-4 rounded-xl border font-mono text-xs uppercase font-bold transition-all ${activePad === 'laser' ? 'active-pad bg-[#f5e156] text-black border-[#f5e156]' : 'bg-[#181818] text-white border-white/10'}`}>03 // Laser Pulse</button>
                      <button onClick={() => triggerPad('cyber')} className={`synth-pad-btn p-4 rounded-xl border font-mono text-xs uppercase font-bold transition-all ${activePad === 'cyber' ? 'active-pad bg-[#f5e156] text-black border-[#f5e156]' : 'bg-[#181818] text-white border-white/10'}`}>04 // Cyber Burst</button>
                    </div>
                  </div>
                )}
                ...
                {/* 06: Magnetic Orbit Nodes */}
                {activeId === 6 && (
                  <div className="w-full h-[360px] bg-[#0d0d0d] rounded-3xl border border-white/10 p-8 flex flex-wrap items-center justify-center gap-6 relative overflow-hidden">
                    {Array.from({ length: 9 }).map((_, idx) => (
                      <motion.div
                        ...
                        animate={{
                          scale: activeNode === idx ? 1.35 : 1,
                          y: [0, -8, 0]
                        }}
                        transition={{
                          scale: { type: 'spring', stiffness: 350 },
                          y: { repeat: Infinity, duration: 2 + idx * 0.3, ease: 'easeInOut' }
                        }}
                        className={`w-14 h-14 rounded-full border-2 flex items-center justify-center font-mono text-xs font-bold cursor-pointer transition-all duration-300 ${
                          activeNode === idx 
                            ? 'bg-[#f5e156] text-black border-white shadow-[0_0_25px_rgba(245,225,86,0.9)]' 
                            : 'bg-[#181818] text-[#f5e156] border-[#f5e156]/30 shadow-md'
                        }`}
                      >
                        N0{idx + 1}
                      </motion.div>
            ...
            <div className="space-y-6 border-t border-white/10 dark:border-white/10 light:border-slate-200 pt-6">
              ...
              {activeId === 6 && (
                <div className="text-center font-mono text-xs text-[#f5e156] dark:text-[#f5e156] light:text-amber-600 font-bold uppercase animate-pulse">
                  Hover over energy nodes to magnetize! 🧲
                </div>
              )}
